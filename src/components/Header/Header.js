...
function Header() {
  ...
  const [items, setItems] = useState([
    ...
    {
      label:
        (<button className="w-full h-full rounded-sm bg-gray-200" onClick={handleLogOut}>
                ...
                {(user?.taiKhoan)
                  ? (
                    ...
                    <Dropdown placement="bottomLeft" arrow menu={{items}}>
                      <span className=" cursor-pointer text-sm text-login capitalize">{user?.hoTen}</span>
                    </Dropdown>
                  ) : (
                    // Login
                    <Link className="text-login text-sm" to="/Login">Đăng Nhập</Link>)
                }
              </div>
              {/* Toggle Button */}
              <button
